...
export class SettingsController {
  ...
  async exportData(req: AuthRequest, res: Response): Promise<Response> {
    try {
      const [transactions, budgets, goals, cards] = await Promise.all([
        ...
        prisma.creditCard.findMany({  where: { userId: req.userId } }),
      ])
      return res.json({
